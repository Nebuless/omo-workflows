...
const IDLE_TIMEOUT_MS =
  Number(process.env.CE_LIGHT_WEB_IDLE_TIMEOUT_MS) || 30 * 60 * 1000;
const LIFECYCLE_CHECK_MS =
  Number(process.env.CE_LIGHT_WEB_LIFECYCLE_CHECK_MS) || 60 * 1000;
const WAIT_TIMEOUT_MS =
  Number(process.env.CE_LIGHT_WEB_WAIT_TIMEOUT_MS) || 30 * 1000;
...
function resolveOwnerPid() {
  ...
  try {
    const grandparent = Number(
      execFileSync("ps", ["-o", "ppid=", "-p", String(parentPid)], {
        encoding: "utf8",
        stdio: ["ignore", "pipe", "ignore"],
      }).trim(),
    );
...
function newestScreen(options) {
  ...
  const files = fs
    .readdirSync(options.screensDir)
    .filter((file) => file.endsWith(".html"))
    .map((file) =>
      containedRealPath(
        options.screensDir,
        path.join(options.screensDir, file),
      ),
    )
    .filter(Boolean)
...
const WAITING_HTML =
  "<h1>Waiting for a page...</h1><p>The agent will update this page when a screen is ready.</p>";
...
function encodePagePath(page) {
  if (typeof page !== "string" || !page.startsWith("/") || page === "/")
    return "/";
  return `/${page
    .slice(1)
    .split("/")
...
function annotateBoot(origin, page = "/") {
  const servedPage = encodePagePath(
...
function annotateDocument(options, origin) {
  ...
  return annotateScreen(
    fs.readFileSync(screen, "utf8"),
    origin,
    pageForScreen(options, screen),
  );
}
...
function renderPage(options, origin) {
  ...
  return isFullDocument(html)
    ? injectRefresh(options, html)
    : wrapFragment(options, html);
}
...
function parseAnnotation(raw, options) {
  ...
  const textSnippet =
    typeof body.textSnippet === "string" ? body.textSnippet : null;
  const rect =
    body.rect && typeof body.rect === "object" && !Array.isArray(body.rect)
      ? body.rect
      : null;
  return {
    id: randomUUID(),
...
async function start(options) {
  ...
  if (
    running &&
    Boolean(running.annotate) === options.annotate &&
...
async function serve(options) {
  ...
  function endSession() {
    ...
    if (publishedInfo && options.infoFile) {
      ...
      try {
        fs.writeFileSync(
          options.infoFile,
          `${JSON.stringify(publishedInfo, null, 2)}\n`,
        );
      } catch {
  ...
  function flushHeld() {
    ...
    for (const item of heldQueue) {
      ...
      if (annotationStates.get(item.id) === "held")
        annotationStates.set(item.id, "queued");
    }
    heldQueue.length = 0;
  }
  ...
  function authorized(req) {
    return requestCredentials(req, cookieName).some((candidate) =>
      tokenMatches(candidate, sessionToken),
    );
  }
  ...
  function requestOrigin(req) {
    ...
    if (typeof host === "string" && HOST_HEADER.test(host))
      return `http://${host}`;
    ...
    if (options.annotate) {
      ...
      if (req.method === "GET" && urlPath === "/") {
        ...
        if (isDocumentNavigation(req)) {
          ...
          serveAnnotateDocument(
            req,
            res,
            renderPage(options, requestOrigin(req)),
            renderedKey,
          );
          return;
        }
      ...
      if (req.method === "GET") {
        ...
        if (!filePath) return;
        if (
          contentType(filePath) === CONTENT_TYPES[".html"] &&
