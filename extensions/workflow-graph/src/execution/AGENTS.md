...
- Await native session flush before dispatch, gate answer, or external design-review event progression. Both synchronous legacy flush and `Promise<void>` receipts must propagate failure; missing flush capability disables launches.
- Append cumulative same-run definitions with fresh node IDs. Preserve completed nodes byte-for-byte.
- Match native run identity and definition fingerprint before admission or amendment recovery. Reject stale descriptor or composition identity before dispatch.
...
- Predeclared `composeStagedPrograms()` uses one staged-program instance, artifact root, checkpoint stream, and native run. Namespace stage waves, nodes, gates, answers, answer modes, and external keys; pass full isolated context to each child. Validate production plans through shared composition validation, including unique destinations per transition and cumulative native node limits. Map admitted final values only through declared RFC 6901 mappings, including nested escaped object/array paths without mutating caller inputs. A non-preapproved boundary exposes `continue` or `stop`; composition is not terminal-run continuation.
